...
def test_record_skipped_roundtrip() -> None:
    event = RecordSkipped(control_id="m-99")
    decoded = decode_stats_event(encode_stats_event(event))
    assert decoded == event
...
def test_events_are_disambiguated_by_tag() -> None:
    ...
    blobs = [
        ...
        encode_stats_event(RecordSkipped(control_id="m-1")),
    ...
    assert isinstance(decoded[1], RecordSkipped)
    assert isinstance(decoded[2], ProducerHeartbeat)
    assert isinstance(decoded[3], WriterHeartbeat)
    assert isinstance(decoded[4], ShutdownEvent)
