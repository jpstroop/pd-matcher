...
def _vault_pair_for_build(
    marc: MarcRecord,
    cce: IndexedNyplRegRecord,
    candidate: CandidateMatch,
) -> PairInsert:
    """Project a scored vault pair into a :class:`PairInsert` for build-queue rows.

    Mirrors the projection ``vault-into-queue`` uses so the two paths produce
    structurally identical rows for the same ``(marc, cce)`` pair.
    """
    ...
    *,
    ...
    with NyplIndexLookup(index_path) as lookup:
        resolved, summary = resolve_vault_pairs(
            ...
            build_pair=_vault_pair_for_build,
        )
    _LOGGER.info(
