...
``data/candidates/`` into a single ``data/vault_marcs.xml``. The output is
the durable bibliographic context for the labeled linkage so downstream
consumers can reproduce or refine the dataset without access to Princeton's
full bibdata dump.
