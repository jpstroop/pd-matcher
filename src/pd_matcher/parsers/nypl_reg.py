...
def _text(element: _Element | None) -> str | None:
    """Return stripped text from ``element`` or ``None`` if absent/empty."""
...
def _collect_publisher_names(entry: _Element) -> tuple[list[str], list[str]]:
...
def _collect_publication_places(entry: _Element) -> list[str]:
    """Return all ``<pubPlace>`` text values nested under any ``<publisher>``."""
    out: list[str] = []
    for publisher in entry.iterfind("publisher"):
        for place_elem in publisher.iterfind("pubPlace"):
            value = _text(place_elem)
            if value is not None:
                out.append(value)
    return out


def _collect_explicit_claimants(entry: _Element) -> list[str]:
...
def _build_record(entry: _Element) -> NyplRegRecord | None:
    ...
    regnum = entry.get("regnum") or _text(entry.find("regNum"))
    ...
    if reg_date is not None:
        ...
        reg_date_elem = entry.find("regDate")
        reg_year = _year_from_text(_text(reg_date_elem)) if reg_date_elem is not None else None

    author_name = _text(entry.find("author/authorName"))
    edition = _text(entry.find("edition"))
    publisher_names, claimants_from_pub = _collect_publisher_names(entry)
    publication_places = _collect_publication_places(entry)
    explicit_claimants = _collect_explicit_claimants(entry)
...
def iter_nypl_reg_records(path: Path) -> Iterator[NyplRegRecord]:
...
def iter_nypl_reg_directory(root: Path) -> Iterator[NyplRegRecord]:
    ...
    for xml_path in sorted(root.rglob("*.xml")):
        yield from iter_nypl_reg_records(xml_path)
